    ...
    def get_list(self, request, **kwargs):
        if 'uuid' in request.GET:
            ...
            if RingGroup.objects.filter(ring_group_uuid=request.GET['uuid']).count() == 1:
               # kwargs["pk"] = RingGroup.objects.filter(ring_group_uuid=request.GET['uuid'])
                print(red)
                return super(RingGroupResource, self).get_detail(request, **kwargs)
            else:
               # kwargs["pk"] = 0
                ...
                response = self.error_response(request, data_res, response_class=HttpBadRequest)
                raise ImmediateHttpResponse(response=response)
        return super(RingGroupResource, self).get_list(request, **kwargs)
